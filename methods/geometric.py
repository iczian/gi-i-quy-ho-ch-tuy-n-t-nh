    ...
    for pt in giao_diem:
        ...
        for (a1, a2, dau, b) in ds_rb:
            ...
            if dau == '=' and abs(val - b) > 1e-7:
                ok = False; break
        if ok:
            da_co = any(abs(pt[0] - d[0]) < 1e-7 and abs(pt[1] - d[1]) < 1e-7 for d in diem_hop_le)
            if not da_co:
                diem_hop_le.append(pt)

    if len(diem_hop_le) == 0:
        return "VO_NGHIEM", [], None, []

    gia_tri_z = [(c1 * pt[0] + c2 * pt[1], pt) for pt in diem_hop_le]
    if loai_hmt == 'max':
        z_toi_uu = max(v[0] for v in gia_tri_z)
    else:
        z_toi_uu = min(v[0] for v in gia_tri_z)

    nghiem_toi_uu = [v[1] for v in gia_tri_z if abs(v[0] - z_toi_uu) < 1e-7]

    nghiem_thuc = []
    nghiem_ao = []
    for pt in nghiem_toi_uu:
        if abs(pt[0]) > M_lon - 100 or abs(pt[1]) > M_lon - 100:
            nghiem_ao.append(pt)
        else:
            nghiem_thuc.append(pt)

    # Check if unbounded objective
    if abs(z_toi_uu) > M_lon / 10:
        return "KHONG_GIOI_NOI", [], None, diem_hop_le

    if len(nghiem_thuc) == 0 and len(nghiem_ao) > 0:
        return "VO_SO_NGHIEM_DUONG_THANG", nghiem_ao[:2], z_toi_uu, diem_hop_le

    if len(nghiem_thuc) == 1 and len(nghiem_ao) > 0:
        return "VO_SO_NGHIEM_TIA", [nghiem_thuc[0], nghiem_ao[0]], z_toi_uu, diem_hop_le

    if len(nghiem_thuc) > 1:
        return "VO_SO_NGHIEM", nghiem_thuc, z_toi_uu, diem_hop_le

    return "NGHIEM_DUY_NHAT", nghiem_thuc, z_toi_uu, diem_hop_le
...
def ve_do_thi_hinh_hoc(c1, c2, loai_hmt, ds_rb, ket_qua, cac_nghiem, z_toiuu, diem_hop_le, z_hien_tai=None):
    ...
    for idx, (a1, a2, dau, b) in enumerate(ds_rb):
        ...
        if a1 == 1 and a2 == 0 and b == 0 and dau == '>=':
            ...
            if abs(a2) > 1e-9:
                y_arr = (b - a1 * x_arr) / a2
                fig.add_trace(go.Scatter(x=x_arr, y=y_arr, mode='lines', name=lbl, line=dict(color=color, width=3.5)))
            else:
                if abs(a1) > 1e-9:
                    fig.add_vline(x=b / a1, line_width=3.5, line_color=color, name=lbl)
    ...
    if cac_nghiem and ket_qua in ("NGHIEM_DUY_NHAT", "VO_SO_NGHIEM", "VO_SO_NGHIEM_TIA", "VO_SO_NGHIEM_DUONG_THANG"):
        ...
        texts = [f"({doi_phan_so(x)}, {doi_phan_so(y)})<br>Z={doi_phan_so(z_toiuu)}" for x, y in zip(opt_x, opt_y)]
        ...
        fig.add_trace(go.Scatter(x=opt_x, y=opt_y, mode='markers+text', name='Nghiệm tối ưu',
                                 text=texts, textposition="top right",
                                 marker=dict(color='#8B2500', size=14, symbol='star', line=dict(color='white', width=1.5))))
