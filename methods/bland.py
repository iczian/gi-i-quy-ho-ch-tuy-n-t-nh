...
def giai_don_hinh_bland(loai_hmt, n_goc, mang_c_goc, dau_cac_bien, ds_rb_vao):
    """Giống Dantzig nhưng chọn biến vào/ra theo chỉ số nhỏ nhất."""
    ...
    while True:
        c = -1; chi_so_min = float('inf')
        for j in range(len(tap_n)):
            if mang_c[j] < -1e-9 and tap_n[j] < chi_so_min:
                chi_so_min = tap_n[j]; c = j
        ...
        for i in range(len(tap_b)):
            if ma_tran_a[i, c] > 1e-9:
                ...
                chuoi_ti_so.append(f"{doi_phan_so(mang_b[i])}/{doi_phan_so(ma_tran_a[i, c])}")
                if ti_so < min_ti_so - 1e-9:
                    min_ti_so = ti_so; r = i
                elif abs(ti_so - min_ti_so) < 1e-9:
                    if tap_b[i] < tap_b[r]:
                        r = i
        ...
        min_ti_so_str = doi_phan_so(min_ti_so)
        str_ti_so_all = ", ".join(chuoi_ti_so)
        log.append(f"**Biến vào:** ${ten_bien_vao}$ (chỉ số nhỏ nhất) — **Biến ra:** ${ten_bien_ra}$ — **Tỉ số:** ${min_ti_so_str}$")
